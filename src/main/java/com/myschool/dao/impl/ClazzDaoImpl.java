...
import com.myschool.dao.inter.ClazzDaoInter;
...
public class ClazzDaoImpl extends BaseDaoImpl implements ClazzDaoInter {
	...
	public List<Clazz> getClazzDetailList(String gradeid, Page page) {
		...
		try {
			StringBuilder sb = new StringBuilder("SELECT c.id cid, c.name cname, g.id gid, g.name gname FROM clazz c, grade g WHERE c.gradeid = g.id");
